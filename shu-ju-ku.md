...
| 类型 | 简介 | 特点 | 应用场景 |
| :--- | :--- | :--- | :--- |
| 字符串string | --- | --- | 常规key-value缓存，计数器：粉丝数、访问量 |
| 哈希hash | 键值对集合,即编程语言中的Map类型 | 适合存储结构化的数据，比如一个对象,并且可以像数据库中update一个属性一样只修改某一项属性值\(Memcached中需要取出整个字符串反序列化成对象修改完再序列化存回去\) | 存储、读取、修改用户属性 |
| 列表list | 链表\(双向链表\) | 增删快,提供了操作某一段元素的API | 最新消息排行等功能\(比如朋友圈的时间线\)，关注列表、粉丝列表，消息队列 |
| 集合set | 一般是hashtable实现，元素不重复 | 1.hashtable实现的话添加、删除,查找的复杂度都是O\(1\) 2.有求交集、并集、差集等操作 | 1.共同好友、共同关注 2.利用唯一性,统计访问网站的所有独立ip 3、好友推荐时,根据tag求交集,大于某个阈值就可以推荐 |
| 有序集合sorted set | 将Set中的元素增加一个权重参数score,元素按score有序排列 | --- | 1.排行榜 2.带权重的消息队列 |
| GEO | --- | --- | 附近的人、附近的酒店 |
...
| 策略 | 描述 |
| :--- | :--- |
| volatile-lru | 从已设置过期时间的数据集中挑选最近最少使用\(least recently used\)的数据淘汰，直到腾出足够空间为止 |
| volatile-ttl | 从已设置过期时间的数据集中挑选剩余时间短\(time to live\)的数据淘汰，直到腾出足够空间为止 |
| volatile-random | 从已设置过期时间的数据集中任意选择数据淘汰，直到腾出足够空间为止 |
| allkeys-lru | 从所有数据集中挑选最近最少使用的数据淘汰，直到腾出足够空间为止 |
| allkeys-random | 从所有数据集中任意选择数据进行淘汰，直到腾出足够空间为止 |
| noeviction | 不淘汰数据，若超过最大内存，返回错误信息 |
| Redis4.0后新增策略 |  |
| volatile-lfu | 从已设置过期时间的数据集中挑选最不经常使用\(least frequently used\)的数据进行淘汰，直到腾出足够的空间为止 |
| allkeys-lfu | 从所有数据集中挑选最不经常使用的数据进行淘汰，直到腾出足够的空间为止 |
...
| 选项 | 同步频率 | 特点 |
| :--- | :--- | :--- |
| always | 每一个写命令都同步 | 会严重降低服务器性能 |
| everysec | 每秒同步一次 | 比较合适，可以保证系统崩溃时只会丢失一秒左右的数据，对服务器性能几乎没有什么影响 |
...
![](https://gitee.com/coldsun233/NotePic/raw/master/img/redis-file_event_handler.png)
...
![](https://gitee.com/coldsun233/NotePic/raw/master/img/redis-enventProcess.png)
...
* **+FULLRESYNC \ \** ：主服务器将对从服务器执行完整重同步操作。这里的 runID 和 offset 都是主服务器的，从服务器会将 runID 保存起来以便下次使用，并且会用 offset 作为自己的初始化偏移量。
* **+CONTINUE** ：主服务器将对从服务器执行部分重同步操作。
* **-ERR** ：主服务器版本低于2.8不支持 psync 命令，从服务器将向主服务器发送SYNC命令，并进行完整同步。

![](https://gitee.com/coldsun233/NotePic/raw/master/img/redis-copy-progress_of_PSYNC.png)

**PSYNC 执行完整重同步和部分重同步时可能遇上的情况**
...
![](https://gitee.com/coldsun233/NotePic/raw/master/img/redis-sentinel.png)
...
使用命令`CLUSTER REPLICATE <node_id>`可以让接收命令的节点成为node\_id所指定节点的从节点，并开始对主节点进行复制。
...
* 每个Redis数据库都保存着一个 watched\_keys 字典，这个字典的 **key 是某个被 WATCH 命令监视的数据库键**，而字典的 **value 则是一个链表，链表中记录了所有监视相应数据库键的客户端**。

   例如右图表示:  
       ❑客户端c1和c2正在监视键"name"。  
       ❑客户端c3正在监视键"age"。  
       ❑客户端c2和c4正在监视键"address"。 ![](https://gitee.com/coldsun233/NotePic/raw/master/img/watched_key.jpg)

* 所有对数据库进行修改的命令，比如SET、LPUSH、SADD、ZREM、DEL、FLUSHDB等等，在执行之后都会对watched\_keys 字典进行检查，查看是否有客户端正在监视刚刚被命令修改过的数据库键，如果有的话，那么会将监视被修改键的客户端的 REDIS\_DIRTY\_CAS 标识打开，表示该客户端的事务安全性已经被破坏。
* 当服务器接收到一个客户端发来的EXEC命令时，如果服务器检测到客户端的 REDIS\_DIRTY\_CAS 标识已经被打开，它会服务器会拒绝执行客户端提交的事务。
